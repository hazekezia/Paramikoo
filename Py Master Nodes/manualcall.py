...
def manualcall():
    ipaddr = list(map(int, input("Pilih Mesin : ").split(",")))
    print("\nAnda memilih Mesin", ipaddr)
    ...
    if (x=="1"):
        ...
        print()
        for iplist in ipaddr:
            ssh_client.connect(hostname=ip_list[iplist-1],username=username,password=password)
            print ("Sukses login ke {0} (Mesin {1})".format(ip_list[iplist-1], iplist))
